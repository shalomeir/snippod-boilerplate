...
DedupePlugin.prototype.apply = function(compiler) {
	compiler.plugin("compilation", function(compilation) {
		...
		compilation.plugin("after-optimize-commons", function(modules) {
			var modulesByHash = {};
			var allDups = [];
		...
		compilation.mainTemplate.plugin("add-module", function(source, chunk, hash, varModuleId, varModule) {
			...
			return this.asString([
				...
				this.indent([
					...
					"commons[" + varModuleId + "] = commons[_m];",
					"break;"
				]),
				"case \"object\":",
				this.indent([
					"// Module can be created from a template",
					"commons[" + varModuleId + "] = (function(_m) {",
					this.indent([
						"var args = _m.slice(1), templateId = _m[0];",
						"return function (a,b,c) {",
						this.indent([
							"commons[templateId].apply(this, [a,b,c].concat(args));"
						]),
						"};"
					]),
					"}(_m));",
		...
		compilation.mainTemplate.plugin("modules", function(orginalSource, chunk, hash, moduleTemplate, dependencyTemplates) {
			...
			source.add("(function(commons) {\n");
			source.add(this.indent([
				"// Check all commons for deduplicated commons",
				"for(var i in commons) {",
				this.indent([
					"if(Object.prototype.hasOwnProperty.call(commons, i)) {",
					this.indent([
						"switch(typeof commons[i]) {",
						"case \"number\":",
						this.indent([
							"// Module is a copy of another module",
							"commons[i] = commons[commons[i]];",
							"break;"
						]),
						"case \"object\":",
						this.indent([
							"// Module can be created from a template",
							"commons[i] = (function(_m) {",
							this.indent([
								"var args = _m.slice(1), fn = commons[_m[0]];",
								"return function (a,b,c) {",
								this.indent([
